...
Seeded at push time and filled from the harness as the runs land — the verdict
discipline at the head of this file is that only a CI verdict counts, so the
table below is the row's authority, not the local suite.

| # | head | what the run decided | outcome |
|---|------|----------------------|---------|
| 1 | `76ca8dd` | first push of `ImprovedBS/CGMY.lean` (52 declarations, written and audited without a local toolchain — this sandbox cannot reach the Lean CDN, so the CI lane is the first thing that ever compiles the module) | _awaiting the harness_ |
