...
| — | *(tooling, no brief)* statement pins + the lint's own falsifiers | arena-ai-coding-agent | *(this PR)* | PENDING — awaiting CI; locally `tests/test_lint.py` 7/7, `lean_lint` OK, oracle 13/13, mutants 4/4. The `elab` pin layer is CI-only by construction, so this row cannot go GREEN from a sandbox. |
