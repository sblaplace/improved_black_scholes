...
| — | *(brief issuance, no theorem)* BRIEF_020 issued: Stage 2's second half — the CGMY law as the `ε ↓ 0` limit of BRIEF_019's compound-Poisson marginals along `εₙ = 2⁻ⁿ`, its tightness, its existence by Prokhorov + Lévy continuity (mathlib's `tendsto_of_tendsto_charFun` *takes* the limit, so the law is constructed and defined as `limUnder`), its CF `cexp (τ L(v))`, and the identification `L = ψ_Y` for `Y ≠ 1` with G1 (the complex-rate Γ integral) as its engine. The route-check at issue turns BRIEF_019 F3's conditional convergence into two absolute ones: at every `ε > 0` the truncated exponent splits *exactly* (`≤ 7.3e−15` over 18 rows) as `A_ε = ∫_{|x|≥ε}(e^{ivx} − 1 − ivx·1_{|x|≤1})ν + iv·C∫_ε^1 x^{−Y}(e^{−Mx} − e^{−Gx})dx`, each piece dominated on `ℝ ∖ {0}` and converging at `ε^{2−Y}` (slopes 1.4996 / 0.9997 / 0.4998 at `Y = ½, 1, 3⁄2`); the limit exponent is finite at `Y = 1` (`|ψ_{1±δ} − L₁| = O(δ)`: the pole is the formula's, not the law's); G1 holds at `z = M − iv` to `2.9e−11` (the real-rate anchor has the same residual); the IBP chains reproduce BRIEF_011's pinned one-sided closed forms to `2.7e−15` / `3.0e−14`; the real-rate drift identity `C∫_0^∞ x^{−Y}(e^{−Mx} − e^{−Gx}) = m^∞` holds to `3.0e−10`; `|L − ψ_Y| ≤ 6.2e−10`; the tightness proxy is monotone in `n` to the limit's value; the `Y ↓ 0` end matches BRIEF_018's `vgLaw` (`0.037518`, `0.158082`); mutants M39–M43 at separations `1.641 … 3.282` plus the unpaired-drift divergence (slope −0.5267, `122.5` at `ε = 2⁻¹⁴`). Correction **C25** records that the `ivx` pieces *pair* into a finite nonzero drift (`d₀ = −1.6411` at `Y = 3⁄2`) rather than cancel. | arena-ai-coding-agent | [#27](https://github.com/sblaplace/improved_black_scholes/pull/27) | **GREEN** @ `0fd79dc`, lean run [36232476133](https://github.com/sblaplace/improved_black_scholes/actions/runs/36232476133) (oracle lane [36232476233](https://github.com/sblaplace/improved_black_scholes/actions/runs/36232476233)) — `lake build` + `#print axioms` audit + statement pins (all 309) + the oracle↔Lean cross-verifier + `lint` + `oracle` all pass on the docs-only diff (`briefs/BRIEF_020_cgmy_law_stage2b.md`, `docs/03`, `docs/04`, `README`, this ledger), green on the first run (both lanes 4m17s). No `.lean` file changed, no pin moved, no lint rule, no workflow; `deferred: {}` untouched. Graded, as the BRIEF_010/012/013/016/017/018/019 issuance rows were, on whether the docs lane stays green. As with those rows, this verdict is about the **documentation**, not a theorem — nothing in `ImprovedBS/CGMYLaw.lean` exists yet, and the theorems' verdict gets its own row when the implementation is graded. Named residual risks carried by the brief: the `Set.range`-vs-`{μ n \| n}` coercion between `isTightMeasureSet_of_tendsto_charFun` and `isCompact_closure_of_isTightMeasureSet`; the `DifferentiableOn` of `z ↦ z ^ (−s)` on the half-plane in the shape the identity theorem wants; and no ready-made `\|e^{−Mx} − e^{−Gx}\| ≤ \|M − G\| x` at the tag. |
| — | *(oracle half only, no theorem)* BRIEF_020 Stage 2b — the CGMY law's route-check landed as a graded test. `experiments/black_scholes.py` gains the `ε ↓ 0` route as its own quadrature (`cgmy_compensated_exponent` `B_ε`, `cgmy_paired_drift` `d_ε`, `cgmy_lk_exponent` `L = B₀ + iv·d₀`, `cgmy_far_drift`, the two one-sided integrals, `gamma_integral_complex_rate` (G1), `cgmy_drift_identity_closed` `m^∞`, `cgmy_lk_exponent_via_one_sided`); `tests/test_bs.py::test_cgmy_law` is the route-check table graded (oracle 25 → 26) and M39–M43 are seeded (mutants 39 → 44, each killed by that test alone). Measured: the decomposition is an identity at every `ε` (`1.55e−14` on a shared mesh — the integrand identity holds node by node, so what is left is rounding — and `5.3e−10` on independent meshes, which is what proves it is not a mesh artifact); both pieces converge at `ε^{2−Y}` (deep-end slopes 1.4996 / 0.9997 / 0.4998, `d₀ = −0.115465 / −0.346002 / −1.641132`); `|L − ψ_Y| ≤ 6.2e−12`; `Y = 1` is finite with the closed form approaching at `O(δ)` from both sides (`err(δ)/err(δ/10) ∈ [9, 11]`); G1 holds to `1.4e−12` with the real anchor no better (`9.9e−13`); the drift identity holds three ways to `2.9e−12` (`m^∞ = −0.116083169 / −1.641663919`); `Re A_ε < 0` along the whole ladder with the CF Lipschitz ratio `≤ 0.9962`; the tightness proxy is monotone in `n` to `0.00471 / 0.00076 / 0.10771 / 0.01788`; the `Y ↓ 0` end matches BRIEF_018's `vgLaw` (`0.037518 / 0.158082`); and the five cheat separations are `1.641132 / 1.641664 / 3.282264 / 2.736107` plus the unpaired drift leg's *divergence* (`122.47` at `ε = 2⁻¹⁴`, slope `−0.5011 = 1 − Y`) against the paired integrand's `−1.6021`. **No `.lean` file, pin, lint rule or workflow changed**: the module is drafted at `briefs/BRIEF_020_CGMYLaw.draft.lean.txt` — all 6 defs, all 18 statements, complete proof terms for the 8 that are citations and 11 `sorry`s in the analytic core — parked outside `ImprovedBS/` with a `.txt` suffix because `lean_lint.py` walks every `*.lean` in the repo and its ratchet allows zero deferred markers (as `ImprovedBS/CGMYLaw.lean` it is `FAIL [RATCHET] total markers 11 > baseline 0`). The `[CGMYLaw]` R1–R4 clause, its four lint mutants, the pins (`309 → 333`), the audit list (`268 → 286`) and the docs queue all wait on those 11 proofs. | arena-ai-coding-agent | [#28](https://github.com/sblaplace/improved_black_scholes/pull/28) | **GREEN** @ `d131b12`, lean run [36238846015](https://github.com/sblaplace/improved_black_scholes/actions/runs/36238846015) (lint, `lake build`, the `#print axioms` audit, the elaborated pins at 309 + 309 with no delta to publish, and the oracle↔Lean cross-verifier, all green in 5m41s; oracle lane [36238846054](https://github.com/sblaplace/improved_black_scholes/actions/runs/36238846054)) — green on the first run, both lanes. Graded on the oracle half only: the verdict certifies that `test_cgmy_law` passes and that M39–M43 are killed by it, **not** a theorem. Nothing in `ImprovedBS/CGMYLaw.lean` exists yet; the module was drafted without a toolchain (there is none in the sandbox and `releases.lean-lang.org` is unreachable from it), so not one line of it has been elaborated, and its verdict gets its own row. Residual risk carried forward: the draft's 11 proofs, and the `Set.range`-vs-coerced-set bookkeeping between `isTightMeasureSet_of_tendsto_charFun` and `isCompact_closure_of_isTightMeasureSet` (read at the tag: the latter's hypothesis *is* the coerced set, so `by simpa using` is the expected bridge). |
| 19 | BRIEF_020 (the CGMY law — Stage 2's second half, with G1 inside it) | arena-ai-coding-agent | [#29](https://github.com/sblaplace/improved_black_scholes/pull/29) | **GREEN** @ `b7d36a1`, lean run [36249141529](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249141529) (oracle lane [36249141507](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249141507)) — `lake build` + `#print axioms` audit (286 entries, every new one on `[propext, Classical.choice, Quot.sound]`, no `sorryAx`) + **statement pins (source 333, elab 333)** + the oracle↔Lean cross-verifier + `lint` (incl. `[CGMYLaw]`) + `oracle` all pass, green on the tip after the two-red elaboration arc below (`lint` 5m35s, `lake build` 5m24s); this row's own documentation commit `a404032` re-graded green (lean run [36249805720](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249805720), oracle lane [36249805742](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249805742), oracle 9m49s on a docs-only diff). `ImprovedBS/CGMYLaw.lean` (1953 lines, 75 declarations): 6 defs (`cgmyCompensatedIntegrand`, `cgmyDriftIntegrand`, `cgmyLKExponent`, `cgmyCpProbability`, `cgmyLaw`, `gammaIntegralComplexRate`) + 18 theorems (`cgmyCompensatedIntegrand_integrable`, `cgmyDriftIntegrand_integrableOn`, `cgmyTruncatedExponent_decomp`, `cgmyTruncatedExponent_tendsto`, `cgmyLKExponent_zero`, `cgmyLKExponent_continuous`, `charFun_cgmyCpProbability_tendsto`, `cgmyCpProbability_isTight`, `cgmyLaw_exists`, `cgmyCpProbability_tendsto_cgmyLaw`, `charFun_cgmyLaw`, `cgmyLaw_unique`, `integral_cpow_mul_cexp_neg_mul_Ioi`, `cgmyDrift_identity`, `cgmyOneSidedExponent_eq`, `cgmyOneSidedCompensated_eq`, `cgmyLKExponent_eq_cgmyExponent`, `charFun_cgmyLaw_eq_cgmyCharFactor`), all in `REQUIRED` + `PROTECTED`. §1 is the decomposition: `cgmyTruncatedExponent_decomp` proves, at every `ε > 0`, `A_ε = ∫_{|x|≥ε}(e^{ivx} − 1 − ivx·1_{|x|≤1})ν + iv·∫_ε^1 cgmyDriftIntegrand` exactly (the `ivx` legs paired through `intervalIntegral.integral_comp_neg`, C25), `cgmyCompensatedIntegrand_integrable` dominates the compensated integrand by `3v²x²ν` on `Icc (−1) 1` (consuming the landed `cgmy_levy_sq_integrable` route through `integrableOn_Ioo_rpow_iff`) and by `2ν` outside it (the landed `cgmy_levy_far_moment` on both tails), and `cgmyTruncatedExponent_tendsto` is two `tendsto_setIntegral_of_monotone`s along `εₙ = 2⁻ⁿ` — no dominated convergence at `λ_ε` (C22), the integrand vanishes at `0` so the union over `n` is the whole line; `cgmyLKExponent_continuous` comes from `continuousAt_of_dominated` near zero and `continuous_of_dominated` outside. §2 is the law: `charFun_cgmyCpProbability_tendsto` (the landed `charFun_cgmyCpLaw` + `Tendsto.cexp`), `cgmyCpProbability_isTight` (`isTightMeasureSet_of_tendsto_charFun`, needing `L` continuous at `0`), `cgmyLaw_exists` (Prokhorov: `isCompact_closure_of_isTightMeasureSet` + `IsCompact.tendsto_subseq` + `ProbabilityMeasure.tendsto_iff_tendsto_charFun` + `tendsto_nhds_unique` — mathlib's `tendsto_of_tendsto_charFun` *takes* the limit, so the law is constructed and then defined as `cgmyLaw := Filter.limUnder atTop (fun n ↦ cgmyCpProbability … (2⁻¹ ^ n) …)` in `ProbabilityMeasure ℝ`), `cgmyCpProbability_tendsto_cgmyLaw` (`tendsto_nhds_limUnder`), `charFun_cgmyLaw : charFun cgmyLaw t = cexp (τ L t)` and `cgmyLaw_unique` (`Measure.ext_of_charFun`) — all on `0 < Y < 2` **including `Y = 1`**. §3 is the identification, **not severed** (F6 unused): G1 `integral_cpow_mul_cexp_neg_mul_Ioi : gammaIntegralComplexRate s z = Γ(s) z^{−s}` for `Re z > 0` by the identity theorem (`hasDerivAt_integral_of_dominated_loc_of_deriv_le` on the half-plane `{Re w > Re z₀ / 2}`, `DifferentiableOn.analyticOnNhd`, `AnalyticOnNhd.eqOn_of_preconnected_of_frequently_eq` anchored on the shipped real-rate `Complex.integral_cpow_mul_exp_neg_mul_Ioi` along the real axis, `Complex.Gamma_ofReal`, `Complex.inv_cpow`); the two one-sided closed forms by one and two `integral_Ioi_mul_deriv_eq_deriv_mul`s (the boundary terms by `squeeze_zero_norm'` + `tendsto_rpow_mul_exp_neg_mul_atTop_nhds_zero`, the Lipschitz bound `‖e^{−zx} − e^{−wx}‖ ≤ ‖z − w‖ x` by `Convex.norm_image_sub_le_of_norm_hasDerivWithin_le` on `convex_halfSpace_re_le`); `cgmyDrift_identity` at the real rate for `Y < 1` (`Real.integral_rpow_mul_exp_neg_mul_Ioi`) and `Y > 1` (one integration by parts + `Real.Gamma_add_one`); `cgmyLKExponent_eq_cgmyExponent` splits `B₀` at `0` (`integral_add_compl`, `integral_comp_neg_Ioi`), sends each leg to its closed form and closes the drift with the identity; and `charFun_cgmyLaw_eq_cgmyCharFactor : charFun cgmyLaw v = cgmyCharFactor C G M Y τ v` for `Y ≠ 1` — the tree's pricing factor is the CF of a law in the tree. Where it stops is part of the claim: the CF is at **real** `v` only, there is no mgf, strip or Esscher tilt at the law, no pricing statement at `cgmyLaw`, no law-level corner at either end; `CGMY.lean`, `CompoundPoisson.lean`, `VGLaw.lean`, `Esscher.lean`, `Corner.lean` and every landed pin are untouched. Counts: pins 309 → 333 in both layers with the 309 pre-existing entries byte-identical (`added 24, changed 0`, the CI-emitted block merged verbatim), audit 268 → 286 (theorems only), `[CGMYLaw]` R1–R4 with lint cheats 52 → 56 and the 5 controls untouched, oracle unchanged at 26 tests / 44 mutants (the route-check landed in PR #28), `deferred: {}` untouched. Locally: `lean_lint` OK (17 files, 437 declarations), `pin_statements --check` OK (333 source + 333 elab), `test_lint.py` 7/7 (56 mutants), `test_pins.py` 12/12. The module was authored, like rows 17 and 18, without a toolchain — the CI comment is the only elaboration channel — and this time the pins delta arrived as its own comment (C24's channel, first use). Correction **C27** (the draft's `cgmyOneSidedCompensated_eq` was stated on `0 < Y < 2` and is false at `Y = 1`; the brief's `1 < Y < 2` is what landed) is this row's. See the CI history below. |
...
### C20 — BRIEF_016's F3 corner-map literals are off in their last digits (BRIEF_016 implementation)

**Date:** 2026-09-25. **Trigger:** implementing BRIEF_016's oracle additions.
F3 prints the Case-4 corner as `G = 2.708131881…`, `M = 5.908131881…`; the map
F3 itself states (`C = 1/ν`, `s = √(θ² + 2σ²/ν)`, `G = (s+θ)/σ²`,
`M = (s−θ)/σ²`) gives `G = 2.708131845707604`, `M = 5.908131845707604`. The
slip is in the brief's trailing digits, not in the map, and it moves nothing:
the implementation pins the MAP and its two cumulant identities
(`C(1/M − 1/G) = θ`, `C(1/M² + 1/G²) = σ² + νθ²`) rather than the truncated
literals, and the anchor prices are identical either way. Recorded because the
briefs are committed up front and corrections are recorded here rather than
edited silently into the original text; the brief stands as issued.

Two implementation choices are worth recording alongside it, both of them
placements rather than changes of content:

* the two routes to the published law are priced **separately** (VG
  `(σ, ν, θ)` and the CGMY corner `(C, G, M)`) and the test asserts they agree
  pointwise, because the F3 map is the one place a silent parameter slip could
  hide; mutants M26 and M29 live on that map, so a wrong `C` or a swapped
  tempering rate is caught by the anchor test alone;
* the *published literals* live in `tests/`, never in `experiments/` (the
  brief's C13 phrase "literal constants in `tests/`"): a constant the oracle
  defines is not an external check.

## CI history for row 17 (PR #25)

| # | head | what the run decided | outcome |
|---|------|----------------------|---------|
| 1–4 | `739ca59` through `6ef367d` (runs 36141698624, 36142589705, 36143491638) | first elaboration of `VGLaw.lean` | RED on `lake build`. The goal states were the spec: `integrableOn_Ioi_comp_mul_left_iff` wants `Ioi (ρ*0)`; `id` survives `unfold mgf`; `Prod.fst - Prod.snd` does not elaborate as `HSub` of projections; `integral_prod_mul` needs the measures named and `SFinite` after `unfold gammaMeasure`; `ring` does not close on `ℂ` |
| 5 | `1fa80d4` (run 36148646057) | the printed goals closed, except two `ring`s after a `field_simp` that had already finished, and an un-annotated `tendsto_const_nhds` | RED, three errors |
| 6 | `310258d` (run 36149305900) | the two `ring`s removed; the constant named `a` | RED, one error: `tendsto_const_nhds` has no parameter `a` |
| 7 | `7bd6166` (run 36149933365) | the constant given an expected type | **build GREEN**, sorryAx audit GREEN, pins RED by design (24 names with no `elab` entry). The failure comment was rejected: tailing every log at 25k characters exceeds GitHub's 65536-character limit, so the delta never posted |
| 8 | `41fbf96` (run 36150901612) | the pin delta posted as its own comment | build GREEN again; pins still RED; the 24-entry `elab_delta` arrived on the PR |
| 9 | `0a9f610` (run 36151499122) | the delta merged verbatim (`added 24, changed 0, elab now 292`) | **GREEN** — every lane, including the oracle lane 36151499027 |

### C21 — BRIEF_018's declaration count, and the M32 magnitude (BRIEF_018 implementation)

**Date:** 2026-09-25. **Trigger:** implementing BRIEF_018.

The brief's prose says "5 defs + 17 audited theorems = 22" and "pins 268 →
268 + 22, audit 237 → 237 + 22". The declaration list printed under that
sentence names 19 theorems, not 17 (`gammaMeasure_mgf` through
`vg_modelFree_parity`). The landed contract is that list: **5 defs + 19
theorems = 24**. Pins move 268 → 292. The audit list moves 237 → 256, not
260: this repository's `#print axioms` step audits theorems, not defs (the
BRIEF_015 precedent — 11 theorems, no defs). The 5 defs are pinned whole, in
both `REQUIRED` and `PROTECTED`.

The corner theorem carries the strip `−M < Im v < G`. The brief's one-liner
omits it; the mathematics does not. `Filter.Tendsto.const_cpow` and
`Complex.log_inv` need the bases in the open right half-plane, which is
exactly that strip, and `vg_corner_cumulant` instantiates it at `v = −iu`.

**M32.** The brief's route-check row quotes the tilted mgf at 1 moving
`0.984025 → 0.946021` under a rate swap. Measured: `0.984025` is the
*untilted* mgf(1), and `0.9460212974415997` is the `θ → −θ` variant. The
leg-swap as prosed (`(G+θ, M−θ) → (M+θ, G−θ)`) measures `1.2061789609599316`
...
### C25 — the near-zero `ivx` pieces pair; they do not cancel (BRIEF_020 issuance)

**Date:** 2026-09-26. **Trigger:** issuing Stage 2's second half (BRIEF_020),
whose §1 has to *state* what the `ε ↓ 0` limit of `cgmyTruncatedExponent` is.
BRIEF_019 F3 and C22 above describe the conditional convergence as "the two
sides' near-zero `i v x` pieces cancel, the remainder is `O(x^{1−Y})` and
dominated". The first clause is wrong in a way an implementer would act on.

* What cancels is the *leading singularity*: `ivx · C x^{−1−Y}` on the right
  and `ivx · C |x|^{−1−Y}` on the left are each non-integrable at `0` for
  `Y ≥ 1`, and their sum over `ε ≤ |x| ≤ 1` is
  `iv · C ∫_ε^1 x^{−Y}(e^{−Mx} − e^{−Gx}) dx`, whose integrand is
  `O(x^{1−Y})` because `e^{−Mx} − e^{−Gx} = O(x)`. That is a **finite, nonzero
  limit**, not zero: measured `d₀ = −0.115465` (`Y = ½`), `−0.346002`
  (`Y = 1`), `−1.641132` (`Y = 3⁄2`) at `(C, G, M) = (0.5, 5, 10)`. It is
  load-bearing — dropping it moves the limit exponent by `|v · d₀|`
  (`1.641132` at `(Y, v) = (3⁄2, 1)`, BRIEF_020 mutant M40), and it is what
  the tree's uncompensated `cgmyExponent` differs from the fully compensated
  Lévy–Khintchine form by, once the far field is added:
  `d₀ + ∫_{|x|>1} x ν(dx) = m^∞ = C Γ(1−Y)(M^{Y−1} − G^{Y−1})` (measured to
  `3.0e−10`). So the tree's exponent *is* a Lévy–Khintchine exponent with a
  specific drift, and the drift is this paired integral.
* The corrected statement, which BRIEF_020 §1 lands as
  `cgmyTruncatedExponent_decomp`: for every `0 < ε ≤ 1`,
  `A_ε(v) = ∫_{|x|≥ε}(e^{ivx} − 1 − ivx·1_{|x|≤1}) ν(dx) + iv · d_ε` with
  `d_ε = C∫_ε^1 x^{−Y}(e^{−Mx} − e^{−Gx}) dx` — an *identity*, not a limit —
  and both pieces are dominated on `ℝ ∖ {0}`, so the `ε ↓ 0` step is two
  monotone-set-integral limits. "Conditional convergence" (C22) remains the
  right description of the *unsplit* integral; the split is what makes it
  provable.
* The one-sided drift leg is the measured cheat: `C∫_ε^1 x^{−Y} e^{−Mx} dx`
  alone is `122.5` at `ε = 2⁻¹⁴`, `Y = 3⁄2`, growing like `ε^{1−Y}` (slope
  `−0.5267`). BRIEF_020's `[CGMYLaw]` R2 forbids integrating either leg alone
  over a neighbourhood of `0`.

BRIEF_019's landed module is unaffected (it takes no limit); F3's diagnosis
that no DCT exists for the unsplit integral stands. The wording "cancel" in F3
and C22 is corrected to "pair into the drift `d_ε`" by this entry.

### C26 — the route-check's residuals are a property of the quadrature, not of the identity (BRIEF_020's oracle half)

**Date:** 2026-09-26. **Trigger:** landing BRIEF_020's route-check as the
graded `test_cgmy_law` (PR #28), which meant re-deriving every row from the
oracle's own primitives rather than trusting the issue-time script's numbers —
a script that was never committed, so its quadrature is not recoverable.

Three of the brief's numbers do not reproduce, and in each case the difference
is the *mesh*, not the mathematics:

* **The IBP-chain rows.** The brief records `2.7e−15` against
  `cgmy_exponent_one_sided` and `3.0e−14` against
  `cgmy_exponent_one_sided_compensated`; the landed route measures `4.9e−13`
  and `2.0e−12`. The landed quadrature's floor there is real and it is
  *fixed*: refining the near-zero panels from 200 to 1600 points per panel
  moves the Gamma-integral residual from `1.56e−11` to `9.72e−13` to
  `6.05e−14` (the expected `n⁻⁴`), so the floor is the panels', and no route
  built on dyadic Simpson reaches `1e−15` without a substitution the brief does
  not describe. The rows still separate every mutant by nine decades, so
  nothing about their force changes.
* **The CF-ladder gaps.** The brief's `4.0e−7` (`Y = ½`) and `1.96e−2`
  (`Y = 3⁄2`) are exactly half the landed `8.1e−7` and `3.98e−2`. The ladder's
  dominant term is the *drift* difference `v·|d_ε − d₀|`, which is linear in
  `v`: the issue-time row ran at `v = 0.5`, the landed one at `v = 1`. Not a
  discrepancy once the `v` is named, and worth naming because it is the one
  number in the table that scales linearly rather than quadratically.
* **M39's slope.** The brief records `−0.5267` for the unpaired drift leg; the
  landed row measures `−0.5011` over `{10⁻⁶, 10⁻⁸}` and `−0.6224` over
  `{10⁻², 10⁻⁴}`. The asymptotic value is `1 − Y = −0.5`, so the brief's
  number is a wider-range fit that still carries the next-order term. The
  landed test asserts the deep-end slope within `2e−2` of `1 − Y`, which is
  the statement that matters (the leg *diverges*), and records `122.47` at
  `ε = 2⁻¹⁴` against the brief's `122.5`.

Two quadrature facts the landed rows depend on, both found by measurement and
both worth stating because they are the kind of thing a re-derivation gets
wrong silently:

1. **A cutoff is not a limit.** Every `eps = 0` row here is dyadic panels down
   to `2⁻⁶⁰` *plus* the analytic `∫₀^inner` of the same integrand from its own
   series. Stopping at `inner = 1e−12` — the floor the landed
   `cgmy_levy_integral_one_sided` uses — leaves `iv·inner^{1−Y}/(1−Y)`, i.e.
   `~1e−6` at `Y = ½`: four decades above anything this brief resolves, and it
   would have shown up as a mysterious `1e−6` disagreement with the closed
   form rather than as an error.
2. **The far field needs grading too.** One uniform Simpson pass on `[1, 60]`
   leaves a *fixed* residual of a few `1e−11` (measured `2.96e−11` at the
   real-rate Γ anchor, identical at 200, 800 and 1600 points per near-zero
   panel, because the near panels are not where it comes from). Doubling
   panels on `[1,2],[2,4],[4,8],…` cut it below `1e−14`. This is the reason the
   first draft of the decomposition row read `6.1e−10` and not `1e−14`: the two
   routes disagreed about the far field, where the integrands are the *same
   function* and only the meshes differ.

One further note, in the brief's favour. The tightness proxy did not reproduce
at first (`0.00444 / 0.00071 / 0.05493 / 0.00898` instead of
`0.00471 / 0.00076 / 0.10771 / 0.01788`). The error was in the test:
`Re φ = e^{τ Re A}·cos(τ Im A)`, and dropping the cosine is a *different*
bound — one that differs from mathlib's by up to a factor of 2, and by exactly
that at `Y = 3⁄2`. With the cosine, all four of the brief's values come out
exactly, which is a useful confirmation that the issue-time route-check was
run rather than estimated.

### C27 — the draft's `cgmyOneSidedCompensated_eq` was stated on `0 < Y < 2`, and that statement is false at `Y = 1` (BRIEF_020 implementation)

**Date:** 2026-09-26. **Trigger:** proving the eleven open `sorry`s of
`briefs/BRIEF_020_CGMYLaw.draft.lean.txt` (PR #29), the draft PR #28 parked
outside `ImprovedBS/`.

The brief's contract (F5) states the compensated one-sided closed form on
`1 < Y < 2`; the draft's *statement* carried `(hY : 0 < Y) (hY₂ : Y < 2)`
instead, with the same right-hand side
`Γ(−Y)·((a − iv)^Y − a^Y + iv·Y·a^{Y−1})`. That is not a harmless widening:

* at `Y = 1` mathlib's `Real.Gamma (−1) = 0` (the tree's own trap, named in
  BRIEF_017: `Real.Gamma` is `0` at the non-positive integers), so the draft's
  right-hand side is `0`;
* the left-hand side `∫_0^∞ (e^{ivx} − 1 − ivx) e^{−ax} x^{−2} dx` is a
  convergent Bochner integral and not zero — measured `−0.43882 − 0.13197 i`
  at `a = v = 1`, with the closed form's own two-sided limit agreeing at
  `O(δ)` (`−0.43849 − 0.13200 i` at `Y = 0.999`, `−0.43916 − 0.13194 i` at
  `Y = 1.001`).

So the draft's theorem, as typed, had a counterexample, and no proof could
have closed it; the pinned draft never reached the elaborator (it was a
`.txt`), which is why the ratchet's "no `sorry` inside `ImprovedBS/`" rule and
the pins' "statements are pinned when they land" rule are the right order. The
landed statement carries `(hY₁ : 1 < Y) (hY₂ : Y < 2)` — the brief's F5 as
written — and spells the last exponent `(a : ℂ) ^ ((Y : ℂ) − 1)` (the draft's
`((Y − 1) : ℂ)` is the same number with the coercion outside; the pin records
the landed spelling). Nothing downstream changes: `cgmyLKExponent_eq_cgmyExponent`
consumes the compensated form only on its `1 < Y` branch and the uncompensated
`cgmyOneSidedExponent_eq` (which *is* on `0 < Y < 1`) on the other; `Y = 1`
is excluded from the identification by hypothesis and is exactly the point the
brief's F4 row says the closed form does not reach while the law does.

Two smaller things the elaboration arc corrected, recorded here because the
next toolchain-free module will meet them: (i) the first-run proofs used
`₊`/`₋` as identifier suffixes, which Lean 4 does not accept in identifiers
(the parser stops at the token and the rest of the proof is silently dropped
— the single largest source of the 33 first-run errors); (ii) a continuation
line beginning with `.aestronglyMeasurable` is Lean 4's *leading-dot*
syntax `(.name …)` resolved against the expected type, not a projection of
the term on the previous line — it resolved to `Real.aestronglyMeasurable`,
which does not exist. Neither is mathematics; both are in the ledger so they
are not repeated.
...
## CI history for row 19 (PR #29)

The module was authored without a toolchain (none in the sandbox; the release
hosts are unreachable from it), so every red below is elaboration at the pinned
mathlib (`v4.34.0`, `autoImplicit = false`) and not mathematics — two red runs,
then one *by-design* red, then green. Mathlib's own source at the tag was
cloned read-only and grepped for every lemma name and signature before use,
which is why the arc is two runs and not five (row 18).

| # | tag | run | what CI said | after |
|---|-----|-----|--------------|-------|
| 1 | `945aa5b` | lean [36246666484](https://github.com/sblaplace/improved_black_scholes/actions/runs/36246666484) | 33 error lines: two `unexpected token '₊'` (the identifier suffix; each one drops the rest of its proof), `le_or_lt` not at the tag (`le_or_gt`), four `have`s on implicit-argument lemmas (`cgmyLevyDensity_nonneg`, `Measurable.aestronglyMeasurable`) that "failed to infer `have` declaration type", `IntegrableOn.congr_fun`'s pointwise goals arriving un-`beta`-reduced or as `(F − G) x` Pi-applications (six `rewrite` failures), `.congr_fun` on an `Integrable`-typed term (`And.congr_fun`, four times, each followed by a consequential `No goals`), dot-notation on a parenthesised `ContinuousOn.mul (…)` term, `add_le_add_right`'s new argument order, a `∫ x in s, f x − g` whose `− g` was parsed *inside* the integrand (`ring failed`), and a type ascription `((∫ …, f x) : ℂ)` that pushes the coercion inside the integrand where `Tendsto.ofReal` keeps it outside (`Type mismatch`, twice) | `4b3b4c2` |
| 2 | `4b3b4c2` | lean [36248277615](https://github.com/sblaplace/improved_black_scholes/actions/runs/36248277615) | 2: `Unknown constant Real.aestronglyMeasurable` at `CGMYLaw.lean:451` and `:1720` — a continuation line starting with `.aestronglyMeasurable` is the leading-dot syntax, resolved against the expected type `ℝ` (C27); everything else in the 1960-line file elaborated | `603e318`, which writes `ContinuousOn.aestronglyMeasurable (…) measurableSet_Ioc` in prefix form |
| 3 | `603e318` | lean [36248648767](https://github.com/sblaplace/improved_black_scholes/actions/runs/36248648767) | **build green, audit green (286, all on the three axioms), pins red by design**: `--elab-check` reported the 24 un-merged `elab` entries, and — first use of C24's channel — posted them as their own comment (`### Elaborated-pin delta on 603e318`) | the delta merged verbatim (`added 24, changed 0, elab now 333`), plus the never-executed `first` fallbacks dropped and `mem_setOf_eq → mem_ofPred_eq` (the deprecation at the tag) |
| 4 | `b7d36a1` | lean [36249141529](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249141529) | **GREEN**: lint (17 files, 437 declarations, `[CGMYLaw]`), build, audit (286), pins (333 + 333), crosscheck; oracle lane [36249141507](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249141507) | this row |
| 5 | `a404032` | lean [36249805720](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249805720) | **GREEN** on the docs-only diff (this row, `docs/03`, `docs/04`, README); oracle lane [36249805742](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249805742) | — |

Two notes worth keeping. First, of the 33 first-run errors not one was a wrong
lemma name or a wrong statement: the names had been checked against the
mathlib source at the tag, and what failed was *elaboration order* — where
Lean puts a coercion, when it beta-reduces, which term a dot-notation
projects from, what `∫ x in s, …` absorbs. The remaining warnings on the module
are the deprecations the tag itself introduces (`push_neg`, which
`Pricing.lean` also carries) and unused-hypothesis linters on statements whose
hypotheses are the brief's contract. Second, run 3 is the row-18 pattern
working as designed: the pins layer was red exactly because the CI-emitted
block had not been merged yet, and this time the block arrived through the
comment channel C24 built, so the merge was a file save and one command.
