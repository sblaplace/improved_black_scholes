...
theorem strikeTransform_eq {S x u α : ℝ} (hS : 0 ≤ S) (hα : 0 < α) :
  ...
  have h_integral_Iic : (∫ k : ℝ, Complex.exp (Complex.I * ↑(u * k)) * ↑(Real.exp (α * k)) *
    ...
    have h_eq : (fun k : ℝ => Complex.exp (Complex.I * ↑(u * k)) * ↑(Real.exp (α * k)) *
      ...
      fun k => (Set.Iic x).indicator (fun k => Complex.exp (Complex.I * ↑(u * k)) * ↑(Real.exp (α * k)) *
        ↑(max (S * Real.exp x - S * Real.exp k) 0)) k := by
      funext k; exact h_f_eq_indicator k
    conv_lhs => rw [h_eq]
  ...
  have h_exp_b2 : ∀ k : ℝ, Complex.exp (((↑(α + 1) + ↑u * Complex.I : ℂ)) * ↑k) =
    ...
    intro k
    have h_exp_b : ((↑(α + 1) + ↑u * Complex.I : ℂ)) * ↑k
        = (↑(α * k) + ↑k) + Complex.I * ↑(u * k) := by
      push_cast; ring
  ...
  have h_on_Iic : ∀ k ∈ Set.Iic x,
    ...
    rw [h1, h_Sexp k, h_exp_b2 k, h_exp_a_sym k]
    ring
  have h_set_eq : (∫ k in Set.Iic x, Complex.exp (Complex.I * ↑(u * k)) * ↑(Real.exp (α * k)) *
    ...
    refine setIntegral_congr_fun measurableSet_Iic fun k hk => h_on_Iic k hk
  ...
  have h_inv_diff : ((↑α + ↑u * Complex.I : ℂ))⁻¹ - ((↑(α + 1) + ↑u * Complex.I : ℂ))⁻¹
      = (cmDenom α u)⁻¹ := by
    have hb0' : ((α:ℂ) + 1 + ↑u * Complex.I) ≠ 0 := by simpa using hb0
    have hbmma : ((α:ℂ) + 1 + ↑u * Complex.I) - (↑α + ↑u * Complex.I : ℂ) = 1 := by
      push_cast; ring
    push_cast
    rw [inv_eq_one_div, inv_eq_one_div, inv_eq_one_div, h_factor,
      div_sub_div _ _ ha0 hb0', one_mul, mul_one, hbmma]
...
theorem exp_moments_of_exp_tail {μ : Measure ℝ} [IsProbabilityMeasure μ] {b c : ℝ}
  ...
  have hpt : ∀ x : ℝ, Real.exp (b * x) ≤ 1 + Real.exp (c * x) := by
    ...
    · have hmul : b * x ≤ 0 := by
        ...
        rw [h2] at h1
        linarith
      have h1 : Real.exp (b * x) ≤ 1 := Real.exp_le_one_iff.mpr hmul
      linarith [(Real.exp_pos (c * x)).le]
    · push_neg at hx
      have h1 : b * x ≤ c * x := by
        have h2 : 0 ≤ (c - b) * x := mul_nonneg (by linarith) hx.le
        have h3 : (c - b) * x = c * x - b * x := by ring
        rw [h3] at h2
        linarith
...
theorem integrable_dampedModelFreeCall_of_exp_moment {μ : Measure ℝ} [IsProbabilityMeasure μ]
  ...
  obtain ⟨hE1, hI1⟩ := exp_moments_of_exp_tail (b := 1) (c := α + 1 + δ) (by linarith) (by linarith) hTail
  obtain ⟨hMom, _⟩ := exp_moments_of_exp_tail (b := α + 1) (c := α + 1 + δ) (by linarith) (by linarith) hTail
    ...
    have hpay_int : Integrable (fun x => max (S * Real.exp x - S * Real.exp k) 0) μ := by
      ...
      · refine Filter.Eventually.of_forall (fun x => ?_)
        ...
        calc max (S * Real.exp x - S * Real.exp k) 0 ≤ S * Real.exp x := by
              ...
              · linarith [mul_nonneg hS.le (Real.exp_pos x).le, mul_nonneg hS.le (Real.exp_pos k).le]
              · exact mul_nonneg hS.le (Real.exp_pos x).le
          _ ≤ S * Real.exp (1 * x) := by rw [one_mul]
  ...
  have hpay_flat : ∀ k : ℝ, ∫ x, max (S * Real.exp x - S * Real.exp k) 0 ∂μ ≤ S * (1 + M) := by
    ...
    have hconst : ∫ x, S * Real.exp (1 * x) ∂μ = S * ∫ x, Real.exp x ∂μ := by
      ...
      exact integral_congr_ae (Filter.Eventually.of_forall
        (fun x => congrArg Real.exp (one_mul x)))
    have hI1' : ∫ x, Real.exp x ∂μ ≤ 1 + M := by
      refine le_trans ?_ hI1
      exact le_of_eq (integral_congr_ae (Filter.Eventually.of_forall
        (fun x => congrArg Real.exp (one_mul x).symm)))
    calc ∫ x, max (S * Real.exp x - S * Real.exp k) 0 ∂μ
  ...
  have hbound : ∀ k : ℝ, ‖(dampedModelFreeCall μ S r tau α k : ℂ)‖ ≤
    ...
    · have hm1 : k ∈ Set.Iic (0:ℝ) := hk
      have hm2 : k ∉ Set.Ioi (0:ℝ) := fun hm =>
        absurd (Set.mem_Ioi.mp hm) (not_lt.mpr hk)
...
theorem fourierDampedModelFreeCall_eq {μ : Measure ℝ} [IsProbabilityMeasure μ]
  ...
  have hnorm_eq : ∀ k x : ℝ, ‖F k x‖ =
    ...
    have h1 : ‖Complex.exp (↑(u * k) * Complex.I)‖ = 1 := Complex.norm_exp_ofReal_mul_I (u * k)
  ...
  have hintk : ∀ x : ℝ, Integrable (fun k : ℝ => F k x) volume := by
    ...
    have h_dom : Integrable (fun k : ℝ => (Set.Iic x).indicator
      ...
      have hbase : Integrable fun k : ℝ => (Set.Iic x).indicator
          (fun k => Real.exp (α * k)) k :=
      ...
      exact (hbase.const_mul (Real.exp (-r * tau) * S * Real.exp x)).congr
        (Filter.Eventually.of_forall (fun k => by
          ...
          · rw [Set.indicator_of_notMem hk, Set.indicator_of_notMem hk]
            ring))
    have hbound : ∀ k : ℝ, ‖F k x‖ ≤ (Set.Iic x).indicator
    ...
    exact h_dom.mono' hmeas (Filter.Eventually.of_forall hbound)
  ...
  have hunc : Integrable (Function.uncurry F) (volume.prod μ) := hunc0
  ...
  have h_inner : ∀ k : ℝ, (∫ x : ℝ, F k x ∂μ) =
    ...
    intro k
    have hkey : ∫ a : ℝ, (↑(max (S * Real.exp a - S * Real.exp k) 0) : ℂ) ∂μ =
  ...
  calc ∫ k : ℝ, Complex.exp (↑(u * k) * Complex.I) * ↑(dampedModelFreeCall μ S r tau α k)
    ...
    _ = Complex.ofReal (Real.exp (-r * tau)) *
          ((↑S * (cmDenom α u)⁻¹) *
            ∫ x : ℝ, Complex.ofReal (Real.exp (x * (α + 1))) * Complex.exp (↑(x * u) * Complex.I) ∂μ) := by
        have hpt : ∀ x : ℝ, Complex.ofReal (Real.exp (-r * tau)) *
              (↑(S * Real.exp (x * (α + 1))) * Complex.exp (↑(x * u) * Complex.I) * (cmDenom α u)⁻¹) =
            Complex.ofReal (Real.exp (-r * tau)) *
              ((↑S * (cmDenom α u)⁻¹) *
                (Complex.ofReal (Real.exp (x * (α + 1))) * Complex.exp (↑(x * u) * Complex.I))) := by
          intro x
...
theorem carrMadan_eq_modelFreeCall {μ : Measure ℝ} [IsProbabilityMeasure μ]
  ...
  have hmain := cmPriceIntegral_eq_damped_modelFreeCall (S := S) (r := r) (tau := tau)
    (k := Real.log (K / S)) hS hα hc hD hY hδ hcont hdecay hTail
...
theorem gaussianReal_exp_moment {m : ℝ} {v : NNReal} (hv : v ≠ 0) (c : ℝ) :
  ...
  have hvpos : 0 < Real.sqrt (v : ℝ) := by
    have h2 : (0 : ℝ) < (v : ℝ) := by
      by_contra hle
      push_neg at hle
      have h3 : (v : ℝ) = 0 := le_antisymm hle (NNReal.coe_nonneg v)
      exact hv (by exact_mod_cast h3)
  ...
  have hcomp : Integrable
    ...
    have hpt : ∀ z : ℝ, Real.exp (c * m) * (Real.exp (c * Real.sqrt (v : ℝ) * z) * phi z)
        = Real.exp (c * (m + Real.sqrt (v : ℝ) * z)) * phi z := by
      intro z
      rw [mul_add, Real.exp_add]
      ring
    rw [integrable_gaussianReal_iff]
    exact ((integrable_exp_mul_phi (c * Real.sqrt (v : ℝ))).const_mul
      (Real.exp (c * m))).congr (Filter.Eventually.of_forall hpt)
