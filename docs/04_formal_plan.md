...
| ~~BRIEF_014~~ | **LANDED GREEN** — BSM-2 kit item 6: the normalized CGMY → GBM corner. Landed: 17 declarations in `ImprovedBS/Corner.lean` (2 defs + 15 audited theorems, all in `REQUIRED` + `PROTECTED`), statement pins 240 → 257 in both layers with the 240 pre-existing entries byte-identical (`added 17, changed 0, elab now 257`), audit list 211 → 226, the `[CORNER]` route check with five cheats killed by name (40 lint mutants, 5 controls), oracle `test_gbm_corner` — the pre-brief route-check promoted out of `scripts/check_gbm_corner.py` — with mutants M22 (doubled scale) / M23 (dropped drift correction) killed by it, PR #22, lean run 36111570530 (oracle lane 36111570572) — green on the 4th lean run: one red on seven elaboration errors (four tactics that outlived their goal, `by continuity` not solving `ContinuousAt`, `Complex.ofReal ∘ _` opaque to `simpa`, and `simp`'s `‖x‖` / `x ^ (2 : ℝ)` normal forms), two red by design on the empty `elab` block, one green with the CI-emitted block merged verbatim (`benchmarks/LEDGER.md` row 14 and its CI history). Correction C17, recorded at issue, is the reason every statement is a one-sided `𝓝[<] 2` limit: at `Y = 2` the tag's `Real.Gamma (-2)` is *defined* (as `0`), so the false bare statement would not even fail loudly. As issued: at the scale `C_Y = (σ²/2)(2−Y)` the pole cancels (`ε Γ(−Y) = Γ(3−Y)/(Y(Y−1))`, citing BRIEF_013's `cgmyGamma_two_sub_eq`) and the exponent converges pointwise on `−M < Im v < G` to `−(σ²/2)v² + i(σ²/2)(G−M)v`; route A's algebraic normalization `Ψ_Y(v) = ψ_Y(v) + i(r−q−κ_Y(1))v` lands it on the risk-neutral GBM exponent and its factor on BRIEF_005's `gbmCharFactor`, route B's named `θ₀ = (M−G−1)/2` lands it on the zero-carry GBM exponent with no correction at all (consuming `esscher_cgmy_shift` at the shifted rates), and the attainable drift half-width collapses to `(σ²/2)(G+M−1)`. Lands at the factor level: no CGMY law, no price convergence, and no interchange of the `Y`-limit with the Carr–Madan integral | 005, 011, 013 | no — CI only |
| ~~BRIEF_015~~ | **LANDED GREEN** — ledger C13 finding 3, the first queued repair brief: the Pareto witness for `ImprovedBS/Levy.lean`'s tail hypothesis. Landed: 11 theorems in `ImprovedBS/ParetoWitness.lean` (no defs — the law is mathlib's `ProbabilityTheory.paretoMeasure`, finding F1), all in `REQUIRED` + `PROTECTED`, pins 257 → 268 in both layers (`added 11, changed 0`), audit list 226 → 237, the `[PARETO]` route check with four cheats killed by name (44 lint mutants, 5 controls), oracle `pareto_pdf` / `pareto_tail` + `test_pareto_witness` with M24 (density exponent) / M25 (the weak `t^(−r)` constant, killed only by the equality assertions) — PR #23, lean run 36119639468, green on the 3rd run. The tail `μ[x,∞) = t^r·x^(−r)` discharges `htail` with equality, the obstruction is instantiated by citation, C7 is a theorem, and the headline is `levy_tail_hypothesis_satisfiable_iff : (∃ law, htail) ↔ 0 < α`, with a Dirac discriminator showing the hypothesis can fail | 004 | no — CI only |
| ~~BRIEF_016~~ | **LANDED GREEN** — oracle lane run `36126534541` (lean lane `36126534548`); no `.lean` file touched (the lean lane stays green with no diff). Ledger C13 finding 4, the second and last queued repair brief: the **external anchor** plus the **term-structure falsifier**, *Python only*. The anchor is re-scoped from Haug's book (not reachable, and not this repository's lane) to the published test case of **Carr & Madan (1999)** §5, Table 1 — the paper the Fourier lane *is*, cited by `Fourier.lean`/`Pricing.lean` — whose Case 4 is the **`Y = 0` (variance-gamma) corner of the CGMY family** (`C = 1/ν`, `G, M = (s ∓ θ)/σ²`, `1 < M`). Three published put prices (`0.6356`, `0.6787`, `0.7244` at `K = 77, 78, 79`) and the paper's own **wrong** VGPS row (`0.2425`, `0.2299`, `1.5386`) become literal constants in `tests/` (a published wrong answer is a free mutant); the oracle reproduces the right row to `\|diff\| ≤ 3.6e−5` and rejects the wrong row by `0.39…0.81`. The falsifier measures the ATM skew `ψ = ∂σ_BS/∂k` at `k = 0` on two witness sets and fits `ψ ≈ c·τ^(−α)`: **`α_model = 1.0857`** (VG at the published Case-4 numbers) and **`0.9682`** (CGMY `1, 5, 10, 0.7`), against the **cited** market law (`α ≈ 0.36…0.45`: El Amrani–Guyon SPX/SX5E/DAX power-law fits `0.43/0.44/0.45` above three weeks with the short-tenor regime change, Gatheral–Jaisson–Rosenbaum `α ∈ (0.3, 0.5)`); the pre-registered bands `[0.90, 1.15]` vs `(0.30, 0.50)` are **disjoint**, and the RED — the exponential-Lévy direction, taken alone, is on the wrong side of the market's decay rate — is landed in docs/02 §A1. Correction **C19** records that C13's own parenthetical (`τ^(−1/2)`) is wrong: the cited large-time rate for exponential-Lévy models is `O(τ^(−1))` (Figueroa-López–Forde–Jacquier), which the measurement confirms — so the family's decay *rate* is theory-correct and the failure is **magnitude**. Landed: oracle additions `vg_exponent`, `cgmy_zeroth_corner_map`, `cgmy_zeroth_exponent`, `cgmy_zeroth_forward_exponent`, `carr_madan_by_exponent`, `implied_vol_bs`, `atm_skew`, `power_law_exponent` (plus `power_law_fit`, so the test's canary can fit a prescribed `τ^(−1/2)` skew) with the published literals in `tests/`, never in the oracle; the new `test_term_structure_anchor` (A1–A5, F1–F4, the canary); mutants **M26–M29**; test count 22 → 23, mutants 26 → 30. No Lean: pins stay 268, audit 237, lint 44/5, `deferred: {}`. Correction **C20** records a last-digit slip in the brief's F3 literals for `(G, M)` (the map itself and every measured number are unaffected) | none | Python only — the oracle lane; the lean lane green with no `.lean` diff |
| BRIEF_017 | **ISSUED** — the **CGMY-law feasibility brief** the queue asked for: the audit, the route call, and the specification of the first law brief. Audit at rev `5ed2965` (`v4.34.0`): mathlib has **no** Lévy–Khintchine, infinitely-divisible, stable-law, subordinator, convolution-semigroup or Bochner/positive-definite machinery — but it does ship the four layers a law can be *built* from: the **Gamma law** (`gammaMeasure` + `gammaPDFReal`, a real probability measure), the **Γ-integral identities** (`integral_rpow_mul_exp_neg_mul_Ioi` with a real rate, `Real.Gamma_add_one`, `Real.Gamma_zero`/`Gamma_neg_nat_eq_zero`), the **moment-generating layer** (`mgf`, `cgf`, `integrableExpSet`, `analyticOn_mgf`, `iteratedDeriv_mgf`, and `complexMGF`, which *is* the characteristic function on the imaginary axis), and the **convergence layer** (`charFun_conv`, `Measure.ext_of_charFun`, `isTightMeasureSet_of_tendsto_charFun`, `ProbabilityMeasure.tendsto_of_tendsto_charFun`, `isCompact_closure_of_isTightMeasureSet` (Prokhorov), `MeasureTheory.mconv`). **Route call:** the family's `Y ↓ 0` corner is the **variance-gamma law** = the difference of two independent Gamma laws (Madan–Seneta; the CGMY Lévy density at `Y = 0` is `C e^{−Mx} x^{−1}`), its mgf is `(M/(M−u))^{Cτ}(G/(G+u))^{Cτ}` on the landed strip `(−G, M)`, the corner limit's Γ pole cancels through `Real.Gamma_add_one` alone (`Γ(−Y)·Y = −Γ(1−Y) → −1`), and the law is the *published* one of BRIEF_016's external anchor under F3's `(C,G,M)` map — so **Stage 1** lands the first CGMY-family probability measure from `gammaMeasure` (mgf/strip, the Esscher tilt at the law level, and the expectation-level twins of items 3 and 5 at a non-Gaussian law), while **Stage 2** (the general-`Y` law by the compound-Poisson truncation limit) is specified and queued, with the ordering constraint on item 6's expectation-level twin recorded. Two traps found and named: `Real.Gamma 0 = 0` and `Real.Gamma (−1) = 0` make a raw evaluation at `Y = 0` or `Y = 1` *silently* the Dirac law (nothing fails; the model is just wrong — the canary is the deterministic price `0.49502543252569353`), and the landed `H_Y` in-range/out-of-range split **degenerates** at `Y = 0` (`H_Y ~ C/Y → ∞`). Route-check (stdlib only): corner `\|ψ_Y − ψ₀\|/Y` → finite constants (O(Y)); the law's mgf vs the **published** VG CF at Carr–Madan Case 4 worst `3.4e−15`; the Γ mgf identity by quadrature worst `1.1e−13` (11 cases incl. `Cτ = 0.125`); θ₀ zeroes the `Y = 0` drift to `5.6e−17`; the tilt map's mgf identity `2.2e−16`; and M30–M33 seeded as its implementation's mutants. Specifies `ImprovedBS/VGLaw.lean` (~16 declarations), a `[VGLaw]` lint check (four clauses: the limit-not-evaluation ban on `Y = 0`, `gammaMeasure`-built with no hand-rolled density, the Γ integral consumed not re-derived, the numéraire condition in the tilt), oracle `test_vg_law`, lint mutants 44 → 48, and the primitive reconciliation with BRIEF_016's `vg_char_exponent`. | 011, 013, 014 | no — CI only |
| BRIEF_018 | **LANDED GREEN** — lean run 36151499122 (oracle lane 36151499027), ledger row 17: `lake build` + `#print axioms` audit (256) + statement pins (elab, all 292) + `lint` (15 files / 345 declarations) + `oracle` all pass; pins 268 → 292 in both layers (`added 24, changed 0`), audit 237 → 256, four `[VGLaw]` lint clauses (44 → 48 cheats), oracle 24/24 with `test_vg_law`, mutants 34/34 with M30–M33. Stage 1 is in the tree (`ImprovedBS/VGLaw.lean`, 5 defs + 19 theorems; the brief's prose counted 22, the listed contract is 24 — correction C21). The spec, as issued: BRIEF_017's Stage-1 contract, made checkable: the **variance-gamma law** as the tree's first CGMY-family probability measure. New module `ImprovedBS/VGLaw.lean`, 5 defs + 17 audited theorems = 22 declarations: `vgLaw` = the two `gammaMeasure`s `(Cτ, M)` and `(Cτ, G)` under `p ↦ p.1 − p.2`, `vgCumulant` (the `Y = 0` real cumulant `C[log(M/(M−u)) + log(G/(G+u))]`), `vgDriftMap`, `vgCornerExponent` (the complex corner target), `vgTilt` (`withDensity` of the exponential over its own mgf); the one new analytical rung is `gammaMeasure_mgf` + `gammaMeasure_exp_integrable`, consuming `integral_rpow_mul_exp_neg_mul_Ioi` (a **real** rate; the complex-rate version stays deferred as G1); then `vgLaw_mgf`/`vgLaw_cgf`/`vgLaw_mgf_one` on the landed strip `(−G, M)`, `vgCumulant_hasDerivAt`, the corner as a `𝓝[>] 0` limit (`vg_corner`, `vg_corner_cumulant`, with `Γ(−Y) = −Γ(2−Y)/(Y(1−Y))` through two `Real.Gamma_add_one` and `HasDerivAt.tendsto_slope_zero_right`), the tilt (`vg_tilt_mgf`, `vg_tilt_cumulant_shift`, `vg_tilt_numeraire` — the numéraire DERIVED by citation of `esscher_tilted_numeraire`, never assumed), and the expectation-level twins `vg_drift_identity` (item 3: `∫ x, S·rexp x ∂(vgTilt …) = S·rexp ((r−q)τ)`) and the `modelFree` bounds/parity at the tilted law (item 5 — the first non-Gaussian law in the tree that discharges BRIEF_009's hypotheses by construction). Guards: the `[VGLaw]` lint check with four clauses (44 → 48 lint mutants, controls 5), oracle `gamma_mgf`/`vg_cumulant`/`vg_mgf`/`vg_drift_map`/`vg_tilted_cumulant`/`vg_esscher_solve`, the committed `test_vg_law` and mutants **M30–M33**; pins 268 → 268 + 22, audit 237 → 237 + 22 (the landed count rules, deviation recorded in the ledger row). Route-check at issue (stdlib only): the Γ integral by quadrature `9.3e−11`, the law's mgf against the **published** no-drift VG factor at BRIEF_016's own `(C, G, M)` map `3.4e−16`, corner `\|ψ_Y − ψ₀\|/Y` finite (`0.1343 → 0.0764` at witness B), the drift solve `θ* = 1.463508761` at witness A (`θ₀ = 1.1`), the tilt identity and the closure `≤ 2.2e−16`, the item-5 Carr–Madan numbers inside the model-free bounds, and the silent-Dirac canary `0.49502543252569353`. Three findings worth carrying: the `Y = 0` drift range **degenerates** (`H_Y ~ C/Y → ∞`, so every `r − q` is admissible and `θ*` is large), the published martingale law is the corner law **translated** by `(r − q − κ₀(1))τ = (r−q+ω)τ` and is a **different** law from the tree's Esscher tilt, and `κ₀(1) = −ω` exactly is the bridge between the two normalizations. | 011, 013, 014, 017 | no — CI only |
| BRIEF_019 | **LANDED GREEN** — Stage 2's first half (BRIEF_017's F5(i)–(ii)): `ImprovedBS/CompoundPoisson.lean`, 5 defs + 12 theorems, all in `REQUIRED` + `PROTECTED`. §1 is the compound-Poisson law `cpLaw λ ρ = Measure.sum (fun n ↦ ENNReal.ofReal (poissonPMFReal λ n) • convPow ρ n)` (`convPow ρ 0 = δ₀`), its mass identity (consuming `hasSum_one_poissonMeasure`) and its CF `cexp (λ (charFun ρ t − 1))` (consuming `charFun_conv` + `integral_sum_measure`/`hasSum_integral_measure`, closing with `NormedSpace.expSeries_div_hasSum_exp` — the `ρ = δ₁` instance is mathlib's own Poisson CF and the route-check's canary, worst 3.5e−16). §2 is the tree's first truncated CGMY law: the landed `cgmyLevyDensity` under `withDensity` on the symmetric set `{ε ≤ \|x\|}` (the module's only density), its mass finite by consuming the landed `cgmy_levy_far_moment` on both tails and bounded on the window `Icc (−1) 1` by `C ε^{−1−Y}`, positive on the window `Ioc ε (ε+1)`; `cgmyJumpLaw = λ_ε⁻¹ • ν_ε` is a probability measure via `Measure.smul_apply` + `ENNReal.inv_mul_cancel`; `cgmyTruncatedExponent = ∫_{\|x\|≥ε}(e^{ivx}−1)ν` is a genuine Bochner integral, and `charFun_cgmyCpLaw` lands `cexp (τ ∫(e^{itx}−1)ν)` at rate `τ λ_ε`, the two `λ_ε` factors cancelling (`ENNReal.inv_mul_cancel` + `integral_smul_measure`). The two conventions are fixed by measurement and enforced by the four `[CPOISSON]` clauses (cheats 48 → 52, 5 controls): symmetric truncation (error `ε^{2−Y}`, slopes 1.4904/0.4947; one-sided diverges `ε^{1−Y}`, slope −0.4798) and no compensator (compensating lands on the *translated* `ψ_Y − iv m^∞`, 1.2e−15 against the landed one-sided compensated closed form). Counts: pins 292 → 309 in both layers with the 292 pre-existing entries byte-identical (`added 17, changed 0`), audit 256 → 268, oracle 24 → 25 (`test_compound_poisson`) with mutants 34 → 39 (M34–M38 killed by that test alone), `deferred: {}` untouched. CI history: five red elaboration runs (56 → 10 → 14 → 7 → 1 error lines; the last was the mirror tail's `x ≤ −1` membership restated) → build + audit green with the pins layer red by design on the 17 un-merged `elab` entries → tip **GREEN** @ `4194219`, lean run [36228682782](https://github.com/sblaplace/improved_black_scholes/actions/runs/36228682782) (oracle lane [36228682768](https://github.com/sblaplace/improved_black_scholes/actions/runs/36228682768)), the CI-emitted block merged verbatim; C24. Corrections C23 (the M34 separator, re-derived) and C24 (the pin bootstrap and the publisher's marker). Stage 2b and G1 stay open; item 6's expectation-level twin stays gated | 011, 017, 018 | oracle + lint yes; Lean CI-only |
| BRIEF_020 | **LANDED GREEN** (PR [#29](https://github.com/sblaplace/improved_black_scholes/pull/29), lean run [36249141529](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249141529), oracle lane [36249141507](https://github.com/sblaplace/improved_black_scholes/actions/runs/36249141507), @ `b7d36a1`; the oracle half landed first as PR [#28](https://github.com/sblaplace/improved_black_scholes/pull/28): `test_cgmy_law` and M39–M43, oracle 25 → 26 and mutants 39 → 44) — `ImprovedBS/CGMYLaw.lean`, 6 defs + 18 theorems, all in `REQUIRED` + `PROTECTED`, §3 **not** severed (F6 unused): the identification `charFun cgmyLaw v = cgmyCharFactor C G M Y τ v` for `Y ≠ 1` is a theorem, so the tree's pricing factor is the CF of a law in the tree. Landed counts: pins 309 → 333 in both layers with the 309 pre-existing entries byte-identical (`added 24, changed 0`), audit 268 → 286 (theorems only), `[CGMYLaw]` R1–R4 with lint cheats 52 → 56, `deferred: {}` untouched; CI history: two red elaboration runs (33 → 2 error lines: identifier suffixes, `le_or_lt`, implicit-argument `have`s, `IntegrableOn.congr_fun`'s goal shapes, a leading-dot continuation line) → build + audit green with the pins layer red by design on the 24 un-merged `elab` entries (the delta posted through C24's channel) → tip GREEN; correction C27 (the draft's `cgmyOneSidedCompensated_eq` was stated on `0 < Y < 2` and is false at `Y = 1`; the brief's `1 < Y < 2` landed). What was issued, for the record — Stage 2's second half (BRIEF_017's F5(iii)–(iv)) with G1 inside it: the CGMY law as the `ε ↓ 0` limit of BRIEF_019's compound-Poisson marginals along `εₙ = 2⁻ⁿ`. New module `ImprovedBS/CGMYLaw.lean`, 6 defs + 18 theorems (17 if the two one-sided closed forms merge): §1 the decomposition `A_ε = B_ε + iv·d_ε` — `B_ε = ∫_{|x|≥ε}(e^{ivx} − 1 − ivx·1_{|x|≤1})ν` dominated by `(x² ∧ 2)ν`, `d_ε = C∫_ε^1 x^{−Y}(e^{−Mx} − e^{−Gx})dx` the *paired* drift — an identity at every `ε > 0` (route-check `≤ 7.3e−15`), and the limit `cgmyLKExponent` `L = B₀ + iv·d₀` by two `tendsto_setIntegral_of_monotone`s (slopes 1.4996 / 0.9997 / 0.4998 at `Y = ½, 1, 3⁄2`); §2 the law: `charFun` of the marginals `→ cexp (τ L)`, tightness (`isTightMeasureSet_of_tendsto_charFun`, needing `L` continuous at 0), existence of the limit law by Prokhorov + `IsCompact.tendsto_subseq` + `tendsto_iff_tendsto_charFun` + `tendsto_nhds_unique` (mathlib has no Bochner theorem; `tendsto_of_tendsto_charFun` takes `μ₀`), `cgmyLaw := Filter.limUnder atTop …` in `ProbabilityMeasure ℝ`, `charFun_cgmyLaw : charFun cgmyLaw t = cexp (τ L t)` and `cgmyLaw_unique` (`Measure.ext_of_charFun`) — all on `0 < Y < 2` **including `Y = 1`** (the pole is the formula's: `|ψ_{1±δ} − L₁| = O(δ)`); §3 G1 `∫ x^{s−1} e^{−zx} = Γ(s) z^{−s}` for `Re z > 0` by the identity theorem (`hasDerivAt_integral_of_dominated_loc_of_deriv_le` + `DifferentiableOn.analyticOnNhd` + `eqOn_of_preconnected_of_frequently_eq`, anchored on the shipped real-rate `integral_cpow_mul_exp_neg_mul_Ioi`; measured `≤ 2.9e−11`), the two one-sided closed forms by one/two `integral_Ioi_mul_deriv_eq_deriv_mul`s (`2.7e−15`, `3.0e−14` against BRIEF_011's pinned oracle forms), the real-rate drift identity `C∫_0^∞ x^{−Y}(e^{−Mx} − e^{−Gx}) = CΓ(1−Y)(M^{Y−1} − G^{Y−1}) = m^∞` (`3.0e−10`), `cgmyLKExponent_eq_cgmyExponent` (`|L − ψ_Y| ≤ 6.2e−10`) and the identification `charFun cgmyLaw v = cgmyCharFactor C G M Y τ v` for `Y ≠ 1`. §3 is severable (F6). Guards: `[CGMYLaw]` R1–R4 (the law is `limUnder`, not a density; the drift is paired; the CF is a theorem of the limit, `cgmyExponent` enters only through the identification; G1 is the identity theorem on the shipped lemma), lint cheats 52 → 56; oracle `test_cgmy_law` with mutants M39–M43 (the unpaired drift leg *diverges* — slope −0.5267, `122.5` at `ε = 2⁻¹⁴` — the dropped drift 1.641, the missing `m^∞` 1.642, the flipped drift 3.282, `Γ(1−Y) → Γ(−Y)` 2.736), oracle 25 → 26, mutants 39 → 44; pins 309 → 333, audit 268 → 286 (landed count rules). Also measured: the tightness proxy `(R/2)∫(1 − Re φₙ)` monotone in `n` to the limit's value, the CF ladder's Lipschitz ratio `≤ 0.999`, and the `Y ↓ 0` end against BRIEF_018's `vgLaw` (`|L − ψ₀|/Y → 0.037518`, `0.158082`). Correction **C25**: the `ivx` pieces *pair* into a finite nonzero drift (`d₀ = −1.6411` at `Y = 3⁄2`); they do not cancel. | 011, 017, 018, 019 | no — CI only |
...
the exact `θ₀ = (M−G−1)/2`. Item 3 landed at the factor level in BRIEF_013; the expectation-level twin of
`integral_spot_mul_phi_eq_forward` is now `vg_drift_identity` at the tilted
variance-gamma law (BRIEF_018). The general-`Y` law is Stage 2, whose first half
has landed as BRIEF_019 (the compound-Poisson mixture and the truncated CGMY jump
law) and whose second half — the `ε ↓ 0` limit, tightness, the law and its CF —
**has landed as BRIEF_020** (`ImprovedBS/CGMYLaw.lean`, PR #29, lean run
36249141529) with G1 (the complex-rate Γ integral) inside it as the
identification's engine: `cgmyLaw` is a `ProbabilityMeasure ℝ` for every
`0 < Y < 2`, its CF is `cexp (τ L)` on all of that range, and
`charFun cgmyLaw v = cgmyCharFactor C G M Y τ v` for `Y ≠ 1`. What stays open
is the law's mgf on the strip and the Esscher tilt *at the law* (items 3/5 at
general `Y`), and any pricing statement at `cgmyLaw`. **BRIEF_017 is issued**
...
tree); Stage 2 is the general-`Y` law by the compound-Poisson truncation limit
(the Lévy-continuity + Prokhorov layer mathlib does ship) — its first half, the
mixture and the truncated family (i)–(ii), has landed as BRIEF_019, and the
`ε ↓ 0` half (2b) has landed as BRIEF_020. Item 6's expectation-level twin is
no longer gated on the law's existence — it now waits on the strip CF at the
law (the brief after BRIEF_020), since the corner statement is about a
sequence of laws in `Y`. The requirement it is
...
has LANDED GREEN** (PR #22, lean run 36111570530): item 6, the normalized
CGMY → GBM corner at `Y ↑ 2` with `C_Y = (σ²/2)(2−Y)` (necessary because
`Γ(−Y)` has a pole), pointwise exponent/factor limits as one-sided
`𝓝[<] 2` statements, and an exact zero-carry Esscher instance. It consumes all
three predecessors it was written against — `cgmyExponent_strip` (BRIEF_011),
`esscher_cgmy_shift` / `esscherDriftMap_zero` / `esscherDriftBound`
(BRIEF_013) and `gbmCharFactor` (BRIEF_005) — and the `[CORNER]` lint check
holds the citation list, with five seeded mutants (G1–G5) for the scale, the
Γ-identity citation, the factor target, the shift citation and the one-sided
filter. The numerical precheck now lives as a committed oracle test,
`tests/test_bs.py::test_gbm_corner` (it started life as
`scripts/check_gbm_corner.py`), and runs with wrong-scaling, fixed-`C`,
rate-only and missing-drift canaries, two of which are seeded mutants (M22,
M23); it does not replace `lake build` or create a CGMY law. Ledger C17 records the correction
of the old bare `Y → 2` / `G,M → σ²/2` wording. C13's two repair briefs
(the Pareto witness for `Levy.lean`'s tail hypothesis — **landed as
BRIEF_015**, PR #23 — and the external anchor / term-structure falsifier, issued as
**BRIEF_016**) are numbered at issue (C13's provisional "013/014" assignment for them is
