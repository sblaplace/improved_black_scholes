...
| BRIEF_016 | **ISSUED** — ledger C13 finding 4, the second and last queued repair brief: the **external anchor** plus the **term-structure falsifier**, as a *Python-only* brief. The anchor is re-scoped from Haug's book (not reachable, and not this repository's lane) to the published test case of **Carr & Madan (1999)** §5, Table 1 — the paper the Fourier lane *is*, cited by `Fourier.lean`/`Pricing.lean` — whose Case 4 is the **`Y = 0` (variance-gamma) corner of the CGMY family** (`C = 1/ν`, `G, M = (s ∓ θ)/σ²`, `1 < M`). Three published put prices (`0.6356`, `0.6787`, `0.7244` at `K = 77, 78, 79`) and the paper's own **wrong** VGPS row (`0.2425`, `0.2299`, `1.5386`) become literal constants in `tests/` (a published wrong answer is a free mutant); the oracle reproduces the right row to `|diff| ≤ 3.6e−5` and rejects the wrong row by `0.39…0.81`. The falsifier measures the ATM skew `ψ = ∂σ_BS/∂k` at `k = 0` on two witness sets and fits `ψ ≈ c·τ^(−α)`: **`α_model = 1.0857`** (VG at the published Case-4 numbers) and **`0.9682`** (CGMY `1, 5, 10, 0.7`), against the **cited** market law (`α ≈ 0.36…0.45`: El Amrani–Guyon SPX/SX5E/DAX power-law fits `0.43/0.44/0.45` above three weeks with the short-tenor regime change, Gatheral–Jaisson–Rosenbaum `α ∈ (0.3, 0.5)`); the pre-registered bands `[0.90, 1.15]` vs `(0.30, 0.50)` are **disjoint**, and the RED — the exponential-Lévy direction, taken alone, is on the wrong side of the market's decay rate — lands in docs/02 §5 at implementation. Correction **C19** records that C13's own parenthetical (`τ^(−1/2)`) is wrong: the cited large-time rate for exponential-Lévy models is `O(τ^(−1))` (Figueroa-López–Forde–Jacquier), which the measurement confirms — so the family's decay *rate* is theory-correct and the failure is **magnitude**. Landed at issue: oracle additions `vg_exponent`, `cgmy_zeroth_corner_map`, `carr_madan_by_exponent`, `implied_vol_bs`, `atm_skew`, `power_law_exponent`; a new `test_term_structure_anchor`; mutants **M26–M29**; test count 22 → 23, mutants 26 → 30. No Lean: pins stay 268, audit 237, lint 44/5, `deferred: {}`. Budgeted one CI run on the oracle lane | none | Python only — the oracle lane; the lean lane must stay green with no `.lean` diff |
