...
| BRIEF_015 | **ISSUED** — ledger C13 finding 3, the first queued repair brief: the Pareto witness for `ImprovedBS/Levy.lean`'s tail hypothesis. Uses mathlib's own `ProbabilityTheory.paretoMeasure` (present at the pinned rev, finding F1), proves the tail `μ[x,∞) = t^r·x^(−r)`, discharges `htail` with equality, instantiates `exp_moment_infinite_of_tail_lower_bound` / `no_drift_makes_spot_integrable` by citation (`[PARETO]`), turns C7's `α ≤ 0` remark into a theorem, and lands the headline `levy_tail_hypothesis_satisfiable_iff : (∃ law, htail) ↔ 0 < α` with a Dirac discriminator showing the hypothesis can fail. New module `ImprovedBS/ParetoWitness.lean`, append-only | 004 | no — CI only |
...
(the Pareto witness for `Levy.lean`'s tail hypothesis — **now issued as
BRIEF_015** — and the Haug / term-structure external anchor, still queued)
