...
| BRIEF_019 | **ISSUED** — Stage 2's first half, BRIEF_017's F5 sub-goals (i)–(ii), made checkable: `ImprovedBS/CompoundPoisson.lean` (name fixed at issue), 5 defs + 12 theorems. §1 is the **compound-Poisson law** of a probability jump measure `ρ` at rate `λ` — `cpLaw λ ρ = Measure.sum (fun n ↦ ENNReal.ofReal (poissonPMFReal λ n) • convPow ρ n)` with `convPow ρ 0 = δ₀`, `convPow ρ (n+1) = convPow ρ n ∗ ρ` — its probability-ness (the mass identity via `Measure.sum_apply` + `hasSum_one_poissonMeasure`) and **its characteristic function** `charFun (cpLaw λ ρ) t = cexp (λ (charFun ρ t − 1))`, consuming `charFun_conv`, `integral_sum_measure`/`hasSum_integral_measure` and the very lemma mathlib's own Poisson `charFun` proof closes with (`NormedSpace.expSeries_div_hasSum_exp`; the `ρ = δ₁` instance is that shipped CF, and the route-check checks the two agree to 3.5e−16). §2 is the tree's first **general-`Y` probability laws**: `cgmyJumpMeasure` = the landed `cgmyLevyDensity` restricted to `{ε ≤ |x|}` (`withDensity`), its mass finite by the landed `cgmy_levy_far_moment` and positive, the normalised `cgmyJumpLaw`, and the Bochner integral `cgmyTruncatedExponent = ∫_{\|x\|≥ε}(e^{ivx}−1)ν` whose CF is the truncated compound-Poisson marginal's. Two conventions are fixed **by measurement** and enforced by the four `[CPoisson]` lint clauses (48 → 52 cheats): the truncation is **symmetric** (two-sided error `O(ε^{2−Y})` — slopes 1.4904 at `Y = ½`, 0.4947 at `Y = 3⁄2`; a one-sided truncation diverges `~ε^{1−Y}` for `Y ≥ 1`, slope −0.4798, no cancellation partner) and carries **no compensator** (compensating everywhere lands on `ψ_Y − iv·m^∞` to 1.2e−15, i.e. a translated law). Its route-check also corrects BRIEF_017's F5(iv): the `ε ↓ 0` step is *not* dominated convergence — `λ_ε → ∞` like `2C/Y·ε^{−Y}` (665216.6 at `ε = 1e−4`), so `2ν` is not integrable near zero and the limit is a conditional-convergence argument, which is why the module lands the truncated exponent and the *next* brief takes the limit. Corner cross-check landed numerically: at `ε = 1e−4` the same route at small `Y` meets BRIEF_018's `vgLaw` with `\|A_ε − ψ₀\|/Y → 0.0375` (`v = 0.5`) / `0.1581` (`v = 2.0`). Guards: `test_compound_poisson` (25th test) with mutants M34–M38. Queued behind it: **Stage 2b** (the sequence `εₙ = 2^{−n}`, tightness, Lévy continuity, the identification `charFun μ_τ v = cexp (τ ψ_Y(v))`, G1 as its analytic engine) and then items 3/5 at general `Y` via mathlib's `Measure.tilted`; item 6's expectation-level twin stays gated. | 011, 017, 018 | oracle + lint yes; Lean CI-only |
...
variance-gamma law (BRIEF_018). The general-`Y` law is still Stage 2, and G1
(the complex-rate Γ integral) stays open. **BRIEF_017 is issued**
...
(the Lévy-continuity + Prokhorov layer mathlib does ship), specified and
queued behind it. Item 6's expectation-level twin stays gated until Stage 2. The requirement it is
